...
real*8 tau(zbins), sigCH4(lambdabins), LWProdTemp(zbins), ColRat(25)
...
do i=1,25
  ...
  TotalRayleigh=0.0d0
  !E IonEnergy = int(Eelec(i))!int(Eion(i))
  !E write(filename,'("./Output/Airglow/ME/LWSpectrum",I0,".dat")') IonEnergy
  !E write(filenameSpec,'("./Output/Airglow/ME/UVSpectrum",I0,"-60.dat")') IonEnergy
  IonEnergy = int(Eion(i))
  write(filename,'("./Output/Airglow/SE/LWSpectrum",I0,".dat")') IonEnergy
  write(filenameSpec,'("./Output/Airglow/UVSpectra/spec",I0,"-",I0,".dat")') IonEnergy,int(angle)
...
!  write(*,*) ''
  do j=1,lambdabins
    ...
    do k=1,zbins
      ...
      LWRay(j)=LWIntensity(j)+(LWProd(k)+LymanA(k))*IntensityRatio(j)*exp(-tau(k))*2e5/(Wavelength(j+1)-Wavelength(j))
...
write(filename,"('./Output/Airglow/SE/ColorRatio',I0,'.dat')") int(angle)
